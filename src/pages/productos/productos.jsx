...
export default function AgregarProducto() {
    ...
    useEffect(() => {
        const fetchAllPuntosVenta = async () => {
            ...
            try {
                const limit = 100;
                const response = await getPointsByCompany(formData.empresa, null, limit);
    ...
    // --- RENDER ---
    const commonInputClasses = "mt-1 w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-[var(--principal-activo)] focus:border-[var(--principal-activo)] dark:bg-gray-700 dark:border-gray-600 dark:text-white";
    const labelClasses = "block text-sm font-medium text-gray-700 dark:text-gray-300";

    return (
        <div className="bg-gray-50 dark:bg-gray-900 p-4 sm:p-6 lg:p-8 min-h-screen">
            <div className="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
                <div className="p-6 text-center bg-[var(--principal)]">
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-8">
                    ...
                    {/* --- SECCIÓN OBLIGATORIA --- */}
                    <fieldset>
                        <legend className="text-lg font-semibold text-gray-800 dark:text-gray-200 border-b pb-2 mb-6">Datos Obligatorios</legend>
                    ...
                    {/* --- SECCIÓN OPCIONAL --- */}
                    <fieldset>
                        <legend className="text-lg font-semibold text-gray-800 dark:text-gray-200 border-b pb-2 mb-6">Datos Opcionales y de Inventario</legend>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                            <div>
                    ...
                    <div className="pt-5">
                        <div className="flex justify-end">
                            <button type="submit" disabled={isLoading} className="w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-sm text-base font-medium text-white bg-[var(--principal)] hover:bg-[var(--principal-shadow)] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--principal-activo)] cursor-pointer disabled:bg-[var(--principal-activo)] disabled:cursor-not-allowed">
