...
// --- Ícono de Spinner para el estado de carga ---
const SpinnerIcon = () => (
...
export default function GetProductsCompany() {
    ...
    // --- EFECTO PARA CARGAR OPCIONES DE FILTRO ---
    useEffect(() => {
        const loadFilterOptions = async () => {
            if (userData?.empresa) {
                try {
                    const [fetchedCategories, fetchedMarcas, fetchedPuntosVenta] = await Promise.all([
        ...
        loadFilterOptions();
    }, [getCategoryEmpresa, getMarcaEmpresa, getPointsByCompany, userData?.empresa]);
                ...
                <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="mb-8 p-6 bg-white rounded-lg shadow-md">
                        <form onSubmit={handleSearch} >
                             {/* SE AJUSTA LA GRILLA PARA INCLUIR EL NUEVO FILTRO */}
                                    ...
                                    <li key={p._id} onClick={() => setSelectedProduct(p)} className="flex items-center justify-between p-4 hover:bg-slate-50 transition-colors duration-150 cursor-pointer">
                                        ...
                                        <div className="hidden sm:flex items-center mx-4 text-center space-x-8">
                                            <div>
                                                ...
                                                <p className="text-md font-medium text-gray-800">{p.stock_disponible}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-gray-500">Precio</p>
                                                <p className="text-md font-bold text-gray-800">${p.precioLista.toFixed(2)}</p>
                                            </div>
                                        </div>
                                        <div className="flex-shrink-0 ml-4">
                                            <span className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${p.activo ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
