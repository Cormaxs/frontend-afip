...
export default function GenerarComprobantes() {
    ...
    const calcularTotales = useCallback(() => {
        ...
        const { items } = state.facturaData;

        items.forEach(item => {
            // 1. Calcular el precio total del ítem (cantidad * precio final unitario)
            const precioFinalItem = item.cantidad * item.precioUnitario;
            
            // 2. Convertir la alícuota de porcentaje a un factor decimal (ej: 21% -> 0.21)
            const tasaIva = item.alicuotaIVA / 100;
        
            // 3. Calcular la base imponible (el precio sin IVA)
            // Se divide el precio final por (1 + la tasa de IVA)
            const baseImponible = precioFinalItem / (1 + tasaIva);
        
            // 4. Calcular el monto del IVA. Es la diferencia entre el total y la base.
            // Esto es más preciso que (baseImponible * tasaIva) por los decimales.
            const montoIva = precioFinalItem - baseImponible;
            ...
            const ivaId = ivaAlicuotasAFIP[item.alicuotaIVA] || 3; // Asumo que ivaAlicuotasAFIP es un objeto que mapea porcentajes a IDs de AFIP
            if (!ivaDetalladoAFIP[ivaId]) {
                ivaDetalladoAFIP[ivaId] = { Id: ivaId, BaseImp: 0, Importe: 0 };
            }
            // Acumulamos los nuevos valores calculados correctamente
    ...
    }, [state.facturaData.items, state.tributos]);

    useEffect(() => {
        calcularTotales();
    }, [state.facturaData.items, state.tributos, calcularTotales]);
    ...
    const handleProductSelect = useCallback((product) => {
        const alicuotaIVA = product.alic_IVA !== undefined && product.alic_IVA !== null 
            ? parseFloat(product.alic_IVA) 
            : 0;
        ...
        const nuevoItem = {
            ...
            cantidad: 1,
            markupPorcentaje: parseFloat(product.markupPorcentaje) || 0,
...
/*
    ...
    const handlePagoChange = (e) => {
        ...
        // Si cambia la forma de pago, establecer el monto al total para Transferencia Bancaria
        if (name === "formaPago") {
            const nuevoMonto = (value === "Transferencia Bancaria" || value === "Efectivo") 
                ? state.facturaData.totales.total 
                : 0;
            ...
            <div className="p-4 bg-gray-100 min-h-screen font-sans">
                <div className="max-w-6xl mx-auto bg-white shadow-2xl rounded-lg">
                    ...
                    <form onSubmit={handleSubmit} className="p-6 space-y-8">
                        ...
                        <fieldset className="border p-4 rounded-lg shadow-md bg-slate-50">
                            <legend className="font-semibold text-lg px-2 text-slate-700">5. Formas de Pago</legend>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                ...
                                {/* Mostrar campo de monto para Efectivo y Transferencia Bancaria */}
                                {(state.facturaData.pagos.formaPago === "Efectivo" || 
                                  state.facturaData.pagos.formaPago === "Transferencia Bancaria") && (
                                    <div>
                                        ...
                                        <input 
                                            type="number" 
                                            name="monto" 
                                            value={state.facturaData.pagos.monto} 
                                        ...
                                        {state.facturaData.pagos.formaPago === "Transferencia Bancaria" && (
                                            <p className="text-xs text-gray-500 mt-1">
                                                Monto transferido: ${state.facturaData.pagos.monto.toFixed(2)}
                                            </p>
                                        )}
                                    </div>
                                )}
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                            <div className="border p-4 rounded-lg bg-slate-800 text-white shadow-md">
                                ...
                                <div className="space-y-2 text-lg">
                                    ...
                                    {state.facturaData.pagos.formaPago === "Efectivo" && (
                                        <div className="flex justify-between border-t border-slate-500 pt-2 mt-2">
                                            <span className="font-bold text-lg">Cambio:</span>
                                            <span className="font-bold text-lg">${(state.facturaData.pagos.monto - state.facturaData.totales.total).toFixed(2)}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
